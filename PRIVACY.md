...
## ChatGPT desktop-app check

PromptMeld does not scan for Word, Outlook, browsers, messaging tools, editors,
or other installed applications. The starter-pack catalogue is the same for
everyone and does not make recommendations from installed software.

The first-use guide checks only whether the required ChatGPT Windows desktop
app has registered its launch protocol or package. This read-only check does
not open ChatGPT, read its content or settings, inspect other installed apps,
or save or transmit a detection result. The official download page opens only
